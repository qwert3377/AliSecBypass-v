...
static const char *AUTH_KEYS[] = {"authcode", "auth_code", "status"};
static const char *EXPIRE_KEYS[] = {"expire", "endtime", "end_time", "is_expired"};
...
static const char *UD_KEYS[] = {"kvipStatusStorageKey", "EDTCActiveDirectAcquireCentral"};
...
static BOOL isUDKey(const char *k)      {
    ...
    for (size_t i = 0; i < sizeof(UD_KEYS)/sizeof(char*); i++) {
        if (strcmp(k, UD_KEYS[i]) == 0) return YES;
    }
    return NO;
}
...
#pragma mark - Hook NSJSONSerialization
