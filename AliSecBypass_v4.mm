//
// GitHub Actions Artifact Downloader v3.6.18
// + File rename in history
//
...
+ (NSArray *)records;
+ (void)clear;
+ (BOOL)renameRecordAtIndex:(NSInteger)index toName:(NSString *)newName;
@end
...
+ (BOOL)renameRecordAtIndex:(NSInteger)index toName:(NSString *)newName {
    ...
    NSMutableArray *arr = [self loadRecords];
    if (index < 0 || index >= arr.count) return NO;
    NSMutableDictionary *rec = [arr[index] mutableCopy];
    NSString *oldPath = rec[@"filePath"];
    if (!oldPath || oldPath.length == 0) return NO;
    ...
    NSString *oldBase = oldFileName.stringByDeletingPathExtension;

    // 提取 build suffix (_#123)
    NSString *buildSuffix = @"";
    NSRange hashRange = [oldBase rangeOfString:@"_#" options:NSBackwardsSearch];
    if (hashRange.location != NSNotFound) {
        buildSuffix = [oldBase substringFromIndex:hashRange.location];
    }

    NSString *newFileName = newName;
    if (buildSuffix.length > 0) newFileName = [newFileName stringByAppendingString:buildSuffix];
    if (ext.length > 0) newFileName = [newFileName stringByAppendingPathExtension:ext];
    ...
    // 更新显示名，保留 build number
    NSString *oldDisplay = rec[@"name"];
    NSString *displaySuffix = @"";
    NSRange dispHash = [oldDisplay rangeOfString:@" (#"];
    if (dispHash.location != NSNotFound) {
        displaySuffix = [oldDisplay substringFromIndex:dispHash.location];
    }
    NSString *newDisplay = newName;
    if (displaySuffix.length > 0) newDisplay = [newDisplay stringByAppendingString:displaySuffix];
...
- (void)shareRecordAtIndex:(NSInteger)index {
    NSArray *records = [GHAHistory records];
    if (index >= records.count) return;
    NSDictionary *rec = records[index];
    NSString *path = rec[@"filePath"];
    NSFileManager *fm = [NSFileManager defaultManager];
...
- (void)renameRecordAtIndex:(NSInteger)index {
    NSArray *records = [GHAHistory records];
    if (index >= records.count) return;
    NSDictionary *rec = records[index];
    NSString *oldName = rec[@"name"];
    // 去掉 build number 后缀作为默认输入
    NSString *defaultName = oldName;
    NSRange dispHash = [oldName rangeOfString:@" (#"];
    if (dispHash.location != NSNotFound) {
        defaultName = [oldName substringToIndex:dispHash.location];
    }

    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"重命名"
                                                                   message:@"输入新文件名（不含扩展名）"
                                                            preferredStyle:UIAlertControllerStyleAlert];
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        if ([GHAHistory renameRecordAtIndex:index toName:newName]) {
            [self.tableView reloadData];
        } else {
            gh_alert(@"重命名失败", @"文件名已存在或无法访问文件");
        }
    }]];
    [alert addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]];
    [self presentViewController:alert animated:YES completion:nil];
...
- (void)viewDidLoad {
    ...
    versionLabel.text = @"GitHub Artifact Downloader v3.6.18";
...
static void gh_init(void) {
    gh_log("INIT", "GitHub Actions Artifact Downloader v3.6.18");
