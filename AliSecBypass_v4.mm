// BLSniff_v7.mm —— 只 hook quickDecrypt: 拿 passphrase + 输入密文 + 输出明文
// 标记 sniff2nd 手动控制: 无=清缓存自杀一遍, 有=纯监听
...
static BOOL safeRead(const void *addr, void *buf, size_t len) {
    vm_size_t out = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)addr, len, (vm_address_t)buf, &out) == KERN_SUCCESS && out == len;
}
...
static void my_quick(void) {
    ...
    if (g_n < 8 && input && input.length > 16 && out && out.length > 16) {
        g_n++;
        slog([NSString stringWithFormat:@"[对#%d] 密文前80: %@", g_n, [input substringToIndex:MIN(80, input.length)]]);
